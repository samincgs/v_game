...
from .sword_arc import SwordArc
...
class Weapon(Item):
    ...
    @property
    def is_gun(self):
        return True if self.projectile_type else False   
    ...
    def attack(self):
        if self.is_gun:
            ...
            self.game.world.projectile_manager.sword_arcs.append(SwordArc(self.game, self.owner, (arc_x, arc_y), random.randint(18, 23), random.randint(7, 15), (random.randint(7, 15), random.randint(7, 15)), self.rotation + 180, decay_rate=random.randint(6, 10)))
        ...
        if self.is_gun:
            if (self.ammo != self.capacity) and (self.max_ammo > 0):
                ...
                if curr_time - self.last_reload >= self.reload_delay:
                    diff = min(self.max_ammo, self.capacity - self.ammo) # use min so when player has almost no ammunition left, the game doesnt crash 
                    self.max_ammo -= diff
                    self.ammo = self.capacity
